...
# ------------------------------------------------------------
#
#	Program Name: calculate_diagnostics_kepert_ens_avg.py
#
# -------------------------------------------------------------
...
GOOGLE_ROOT = os.environ['GOOGLE_ROOT'] # this will vary based on the user's system

# define the font family
mpl.rc('font',family='Arial')

# define the configurations
configs = [sys.argv[1]] # this value is read in from the "run_kepert_analysis.sh" script

# define the time step labels
time_steps = range(1, 17) # in days
time_steps = np.asarray(time_steps, dtype='str')
for t_i, time in enumerate(time_steps):
	time_steps[t_i] = time.zfill(2) # the final result is daily time steps as strings with 2 characters and right justification (e.g. '01', '02', etc.)
...
for c_i, config in enumerate(configs): # loop through each configuration
	...
	# the following input/output paths will vary by user
	...
	if not os.path.exists(output_path):
		os.mkdir(output_path) # if the path does not exist, make it
	
	# calculate surface pressure minimum and surface wind maximum for the configuration (all times included)
	min_sfc_p, max_sfc_wind = calc_p_sfc_ens_avg(config, num_ensembles, time_steps) # this function requires importing output_functions.py
	...
	for t_i, time_step in enumerate(time_steps): # loop through each time step

		# Step 3 (read in the input file...assuming one at this point)
		
		print('Now examining ' + config + '/time step ' + time_step + '...')
		
		# define file name
		file_name = config + '_ens_avg_' + time_step + '.nc'

		# open up the file of radial averages and read in the data
		...
		# define the bounds of the data domain
		...
		for l_i, level in enumerate(levels):
			...
			if np.ma.is_masked(level_v_theta):
				...
				lowest_level = level
				break
		sfc_v_thetas = rad_v_theta[lowest_level_index, :] # at all radii
		sfc_v_rads = rad_v_rad[lowest_level_index, :] # at all radii
		sfc_v_theta = rad_v_theta[lowest_level_index, max_radius_index] # at the lowest level and at the radius of maximum wind
		...
		# calculate the level at which the maximum tangential wind speed occurs (at the radius of maximum wind)
